...
query = """Select count(distinct s.rzdate),d.kid,'所有品类' as clsname,Round(sum(s.counts)*1000,0) as SUMcounts,Round(sum(s.pftotal)*1000,0) as SUMPFTOTAL 
from
...
Select NULL as rzdate,NULL as branchno,NULL as customercode,b.plucode,-b.counts As counts, -b.pftotal as pftotal, -b.yttotal as ystotal
 ...
 Where b.branchno = h.branchno And b.pfreturnno = h.pfreturnno
   And h.rzdate Between '%s' And '%s') s, tskuplu p, tdrpetpcustdetail d
...
for m in month:
    ...
    if m[0] == month[0][0]:
        data.to_csv('c:\\Users\\150972\\Desktop\\working\\所有品类.csv',index=False)
    else:
        data.to_csv('c:\\Users\\150972\\Desktop\\working\\所有品类.csv',mode='a',header=False, index=False)
