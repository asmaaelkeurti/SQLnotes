...
Select c.kid,
       c.orgcode,
       c.custcode,
       round(sum(s.counts*e.lsavghjprice)*1000,0) as ExOfficeCOGS,
       count(distinct s.plucode) as ExOfficeUniSKU
  From (
...
def asmaa():
    for m in month:
        ...
        data['EXOFFICECOGS'] = data['EXOFFICECOGS']/1000.0
        data['date'] = m[0]
    
        if m[0] == month[0][0]:
            data.to_csv('C:\\Users\\150972\\Desktop\\working\\office\\EXOFFICE.csv',index=False)
        else:
            data.to_csv('C:\\Users\\150972\\Desktop\\working\\office\\EXOFFICE.csv',mode='a',header=False, index=False)
