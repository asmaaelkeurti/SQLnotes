...
query = """Select count(distinct s.rzdate),d.kid,'除考试中性笔之外所有小类合计' as clsname,Round(sum(s.counts)*1000,0) as SUMcounts,Round(sum(s.pftotal)*1000,0) as SUMPFTOTAL 
from
...
for m in month:
    ...
    if m[0] == month[0][0]:
        data.to_csv('c:\\Users\\150972\\Desktop\\working\\除考试中性笔之外所有小类合计.csv',index=False)
    else:
        data.to_csv('c:\\Users\\150972\\Desktop\\working\\除考试中性笔之外所有小类合计.csv',mode='a',header=False, index=False)
