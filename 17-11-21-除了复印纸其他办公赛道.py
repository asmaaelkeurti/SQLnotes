...
Select c.kid,
       c.orgcode,
       c.custcode,
       round(sum(s.counts*e.lsavghjprice)*1000,0) as ExCPOfficeCOGS,
       count(distinct s.plucode) as ExCPOfficeUniSKU
  From (
...
Select NULL as rzdate,h.branchno,h.customercode,b.plucode,-b.counts As counts, -b.pftotal as pftotal, -b.yttotal as ystotal
 ...
 Where s.branchno = c.orgcode 
   ...
   and n.materialcode = p.materialcode
   and n.lane2017 like '%%办公%%'
...
def asmaa():
    for m in month:
        ...
        data['EXCPOFFICECOGS'] = data['EXCPOFFICECOGS']/1000.0
        data['date'] = m[0]
    
        if m[0] == month[0][0]:
            data.to_csv('C:\\Users\\150972\\Desktop\\working\\office\\ExcludeCopyPaperOfficeChannel.csv',index=False)
        else:
            data.to_csv('C:\\Users\\150972\\Desktop\\working\\office\\ExcludeCopyPaperOfficeChannel.csv',mode='a',header=False, index=False)
